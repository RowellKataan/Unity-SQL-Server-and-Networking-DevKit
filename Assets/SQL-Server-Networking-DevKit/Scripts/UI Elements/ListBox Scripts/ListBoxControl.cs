...
//			1.0.002 : Oct 27, 2016 :	Fixed the SetScroll function to properly display the selected list item (intIndex)
//																in the Scroll Container.
//																Adjusted the delay settings when drawing the drop down list box. (quicker)
//
// ===========================================================================================================
...
public	class						ListBoxControl : MonoBehaviour 
{
	...
	#region "PRIVATE CONSTANTS"
		...
		private const float			SCROLL_DELAY	= 0.12f;
	...
	#region "PUBLIC PROPERTIES"
		...
		public		virtual	int							SelectedValueInt
		{
			get
			{
				if (_intSelectedItem < 0 || _intSelectedList == null || _intSelectedList.Count < 1)
					return -1;
				else
	...
	#region "PUBLIC FUNCTIONS"
		...
		#region "LIST BOX ITEMS"
			...
			public	virtual	void			SelectByValue(string		strValue, bool blnShifted = false, bool blnCtrled = false)
			{
				int i = Items.FindIndex(x => x.Value.ToLower() == strValue.ToLower());
				SelectByIndex(i, blnShifted, blnCtrled);
			}
			public	virtual	void			SelectByValue(int				intValue, bool blnShifted = false, bool blnCtrled = false)
			{
				string strValue = intValue.ToString();
				int i = Items.FindIndex(x => x.Value.ToLower() == strValue.ToLower());
				SelectByIndex(i, blnShifted, blnCtrled);
			}
			public	virtual	void			SelectByText(	string		strText,	bool blnShifted = false, bool blnCtrled = false)
			{
				int i = Items.FindIndex(x => x.Text.ToLower() == strText.ToLower());
				SelectByIndex(i, blnShifted, blnCtrled);
			}
