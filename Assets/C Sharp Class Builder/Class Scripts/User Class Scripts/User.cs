...
//	private	GameManager			_gmm						= null;
		...
		[SyncVar(hook="OnClassSyncChange")]
		private string					_strClassSync		= "";
		private string					_strLastSync		= "";
...
*/
	...
	#region "PUBLIC PROPERTIES"
		...
		public		bool					IsAdmin
		{
			get
			{
				return UserType == UserTypes.AsstAdmin || UserType == UserTypes.Admin;
			}
		}
	...
	#region "PUBLIC FUNCTIONS"
		...
		public						void	SetUpUserObject()
		{
			...
			if (isServer && this.UserID > 0)
					_strClassSync = Crypto.Encrypt(this.Serialize());
		}
	...
	#region "NETWORK FUNCTIONS -- SERVER COMMANDS & CLIENT RPC'S"
		...
		#region "NETWORK EVENT FUNCTIONS"
			...
			{
				_strLastSync	= "";
				if (!isServer && !isLocalPlayer && _strClassSync != "")
				{
					_strLastSync = _strClassSync;
					this.Deserialize(Crypto.Decrypt(_strClassSync));
					this.SetUpUserObject();
				}
			}

			private void						OnClassSyncChange(string strValue)
			{
				if (strValue != _strLastSync)
				{
					_strLastSync = strValue;
					if (!isLocalPlayer)
					{
						this.Deserialize(Crypto.Decrypt(strValue));
						this.SetUpUserObject();
					}
				}
			}
						...
						if (this.UserID > 0)
						{
							...
							else if ((App.UserLogInType == 1 && this.Password == strPassword) || App.UserLoginType == 2)
							{
								...
								if (UserManager.Instance != null)
										...
										App.AddToDebugLog("UserManager does not Exist.");
